...
local pilotClass = require( "pilot" )
...
function scene:createScene( event )
	...
	--this fn starts physics, then a timer jumps to a local fn GravityOff with a delay to trun gravity off.
		physics.start()
		local function GravityOff(event)
			physics.setGravity(0,0)
			local gx, gy = physics.getGravity()
			print( "X gravity: " .. gx .. ", Y gravity: " .. gy )
		end
		local q = timer.performWithDelay(1,GravityOff,1)
	end
	...
	local  function Platform()
		...
		aliensheetR = sprite.newSpriteSheet( "alien-3.png", 60,39) -- ( image.png, spriteWidth, spriteHeight)
		...
		Platforms[3] = display.newRect( a+10,e-(9*d), a+10, c )
		...
		Platforms[4] = display.newRect( 10,e-(12*d), a+10, c )
		...
		Platforms[5] = display.newRect( a + 10,e-(15*d), a+10, c )
		...
		Platforms[7] = display.newRect(10,e-(18*d), a+10, c )
		physics.addBody(Platforms[7], "static", {friction=1, bounce = 0})
		Platforms[7]:setFillColor( 255, 0, 0 )
		group:insert(Platforms[7])
		--[[Swarm[6] = sprite.newSprite(alienspriteSetS)
		physics.addBody(Swarm[6], "dynamic", {friction=0, bounce = 0})
		Swarm[6].x = a
		Swarm[6].y = e-(18*d)
		Swarm[6]:prepare("alienS")
		Swarm[6]:play()
		Swarm[6].type="alien"]]--
		
		Platforms[8] = display.newImage("spacedoor.jpg",10,10 )
		--Platforms[8].xScale = 0.66
		--Platforms[8].yScale = 0.66
		physics.addBody(Platforms[8], "static", {friction=1, bounce = 0})
		--Platforms[8]:setFillColor( 255, 0, 0 )
...
function scene:enterScene( event )
	y =timer.performWithDelay(1000,ChangeLevel,1)
