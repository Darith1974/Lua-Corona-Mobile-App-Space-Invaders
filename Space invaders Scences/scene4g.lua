...
local function Damagefleet()
	...
	print("waypoint 4")
	print("DamageFleetCounter =")
	print(DamageFleetCounter)
	--[[count = 0
	for i = 1,x do
		print("waypoint 5")
		print(fleet[x].isVisible)
		if fleet[i].isVisible == false then
			--physics.removeBody(fleet[x])
			--fleet[x]:removeSelf()
			print("waypoint 6")
			count = count + 1
	...
	if DamageFleetCounter == 6 then
		fleet[FleetStatus]="destroyed"
		DamageFleetCounter =0
		physics.stop()
		local function changeScene(event)
			local options = {effect = "fade", time = 400 }
			storyboard.gotoScene( "scene4i", options)
		end
		ChangeSceneTimer= timer.performWithDelay(1,changeScene,1)
...
function scene:createScene(event)
	...
	local function Aliens()
		...
		--for i =1,NoOfAliens do
			fleet[x] = sprite.newSprite( alienspriteSet)
			fleet[x].x= math.floor(math.random() * (display.contentWidth - 44.1))
			fleet[x].y= 50*(math.random(1,5))
			fleet[x]:prepare("alien")
			fleet[x]:play()
			physics.addBody(fleet[x], "static", {friction=0.3, bounce = 0,density = 0})
			transition.to( fleet[x], { y = display.contentHeight, time = 2000})
			fleet[x].type="destructible"
			group:insert(fleet[x])
		--end
	end
	...
	Viper:prepare("ship")
	Viper:play()
	AlienTimer = timer.performWithDelay(700, Aliens,0)
...
function scene:enterScene( event )
	...
	EnemyMissileTimer = timer.performWithDelay(3800,EnemyMissile , 0)
	-- This timer calls the EnemyMissile fn every 2 s.
	LifeOfViper = timer.performWithDelay(2000,LifeOfShip,0)
	-- This timer calls the LifeOfShip fn every 1s.
	FleetCheck = timer.performWithDelay(999,Damagefleet,0)
	--This fn accesses the state of the enemy fleet.
	--AlienTimer = timer.performWithDelay(100, Aliens,0)
end
